...
// Deduct 5 credits from provider
$creditCost = 5;
...
// Update chat's service_id if not already set
if (!$chat['service_id']) {
    $pdo->prepare("UPDATE chats SET service_id = ? WHERE id = ?")->execute([$serviceId, $chatId]);
}
